...
export const Dropdown = ({
  ...
  return <div ref={ref} className={`relative min-w-[180px] ${className}`} style={{ userSelect: "none" }}>
      <button
    type="button"
  ...
  >
      ...
      {isOpen && <div className="absolute left-0 right-0 z-50 mt-1 bg-[var(--bg-card)] border border-[var(--border-color)] rounded shadow-lg max-h-60 overflow-y-auto">
          {isSearchable && <div className="p-2 border-b border-[var(--border-color)]">
              <input
    type="text"
    value={searchTerm}
  ...
  >
                <span>{opt.label}</span>
                {isSelected(opt.value) && <svg style={{ width: "14px", height: "14px", color: "var(--accent-color)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                  </svg>}
              </li>)}

            {Object.keys(groups).map((groupName) => <li key={groupName} className="list-none">
                <div className="px-3 py-1 text-xs font-bold text-[var(--text-muted)] bg-[var(--bg-dashboard)]/50 uppercase tracking-wider">
                  {groupName}
                </div>
                <ul>
                  {groups[groupName].map((opt) => <li
    key={opt.value}
    role="option"
    aria-selected={isSelected(opt.value)}
    onClick={() => handleSelect(opt.value)}
  ...
  >
            ...
            {filteredOptions.length === 0 && <li className="px-3 py-3 text-sm text-[var(--text-muted)] text-center">
                No results found
              </li>}
          </ul>
        </div>}
