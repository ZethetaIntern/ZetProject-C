...
const columns = [
  ...
  { key: 'price', header: 'Price ($)', sortable: true, render: (row: any) => <strong>${row.price}</strong> },
  { key: 'change', header: 'Change', sortable: true, render: (row: any) => <span style={{ color: row.change >= 0 ? 'var(--color-up)' : 'var(--color-down)' }}>{row.change}%</span> },
];
