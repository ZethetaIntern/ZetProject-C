...
export function DataTable<T extends Record<string, any>>({
  ...
  return (
    <div className={`flex flex-col gap-3 h-full overflow-hidden ${className}`}>
        ...
        <table className="w-full text-left border-collapse text-xs select-text">
          ...
          <tbody className="divide-y divide-[var(--border-color)] text-[var(--text-primary)]">
            {paginatedData.map((row, idx) => (
              <tr key={idx} className="hover:bg-[var(--bg-dashboard)]/30 transition-colors">
                {columns.map((col) => (
                  <td key={String(col.key)} className="px-4 py-2 text-[var(--text-primary)]">
