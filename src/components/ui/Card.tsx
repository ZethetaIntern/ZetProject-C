...
export const Card: React.FC<CardProps> = ({
  ...
  return (
    <div
      className={`bg-[var(--bg-card)] rounded-lg shadow-sm flex flex-col overflow-hidden transition-all duration-200 ${
        showBorder ? 'border border-[var(--border-color)]' : ''
      } ${className}`}
    ...
    >
      ...
      {/* Card Footer */}
      {!isCollapsed && footer && (
        <div className="px-4 py-2 bg-[var(--bg-dashboard)]/60 border-t border-[var(--border-color)] text-xs text-[var(--text-secondary)]">
          {footer}
        </div>
      )}
    </div>
