...
export const ColorPicker: React.FC<ColorPickerProps> = ({
  ...
  return (
    <div ref={ref} className={`relative inline-block ${className}`}>
      ...
      {isOpen && (
        <div className="absolute left-0 z-[100] mt-1 p-3 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl w-48">
          ...
          <div className="grid grid-cols-4 gap-2 mb-3">
            {presets.map((p) => (
              <button
                ...
                className={`w-8 h-8 rounded border transition-transform hover:scale-105 cursor-pointer ${
                  color === p ? 'border-[var(--text-primary)] ring-1 ring-[var(--text-primary)]' : 'border-[var(--border-color)]'
                }`}
