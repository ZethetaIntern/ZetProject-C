...
import { useDashboardStore, UserRole, ThemeType } from './store/dashboardStore';
...
// React class component for the global-level Error Boundary
...
const SessionWarningBanner: React.FC = () => {
  ...
  return (
    <div className="w-full bg-[var(--color-down-bg)] border-b border-[var(--color-down)] text-[var(--color-down)] text-[11px] font-bold px-4 py-1.5 flex items-center justify-between z-[1000] select-none">
...
export const AppContent: React.FC = () => {
  ...
  const [roleInput, setRoleInput] = useState<UserRole>('Portfolio Manager');
  const [activeTab, setActiveTab] = useState('dashboard');

  useEffect(() => {
    // Tick auth session every second
    const sessionId = setInterval(() => tickSession(), 1000);

    // Sync WebSocket simulated state
  ...
  // Dropdown options for widget selection
  const widgetOptions: DropdownOption[] = [
  ...
  const handleAddWidgetSelect = (type: string) => {
    if (!type) return;
    addWidget(type);
  };
  ...
  return (
    <div className="flex flex-col h-screen select-none" style={{ backgroundColor: 'var(--bg-dashboard)', color: 'var(--text-primary)' }}>
      {/* Top Banner Alert (WS Offline warning) */}
      {!wsConnected && (
        <div className="w-full bg-[var(--color-warning-bg)] border-b border-[var(--color-warning)] text-[var(--color-warning)] text-[11px] font-bold px-4 py-1.5 flex items-center justify-between z-[1000] select-none">
          <div className="flex items-center gap-2">
            <svg style={{ width: '14px', height: '14px' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
      ...
      <header className="h-14 bg-[var(--bg-header)] border-b border-[var(--border-color)] flex items-center justify-between px-6 z-40 select-none">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {wsConnected ? (
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--color-up)]" />
            ) : (
              <span className="w-2.5 h-2.5 rounded-full bg-[var(--color-down)] animate-pulse" />
            )}
          </button>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-center md:justify-start gap-3 px-3 py-2 rounded text-xs font-semibold transition-colors cursor-pointer text-left ${
                    active ? 'bg-[var(--accent-color)] text-white' : 'text-slate-400 hover:bg-slate-800'
                  }`}
