...
import mockDataService, { Transaction } from '../services/mockDataService';
import SearchInput from '../components/ui/SearchInput';
import DataTable, { Column } from '../components/ui/DataTable';
import Badge from '../components/ui/Badge';

export const TransactionLogWidget: React.FC = () => {
  const [transactions, setTransactions] = useState<Transaction[]>([]);
  const [searchValue, setSearchValue] = useState('');
  const [lastUpdated, setLastUpdated] = useState<number>(Date.now());

  useEffect(() => {
    // Grab historical pre-populated transactions
    const hist = mockDataService.getTransactions();
    setTransactions(hist);

    // Subscribe to WebSocket live tick updates
        ...
        render: (row) => (
          <span
            className={`font-bold text-[10px] ${
              row.side === 'BUY' ? 'text-[var(--color-up)]' : 'text-[var(--color-down)]'
            }`}
          >
            {row.side}
          </span>
  ...
  return (
    <div className="flex flex-col h-full gap-2 text-xs overflow-hidden">
      {/* Search Header */}
