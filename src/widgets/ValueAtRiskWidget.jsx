...
const CircularGauge = ({ label, value, max, suffix = "%" }) => {
  const radius = 32;
  const strokeWidth = 6;
  const circumference = 2 * Math.PI * radius;
  const percentage = Math.min(100, Math.max(0, value / max * 100));
  const strokeDashoffset = circumference - percentage / 100 * circumference;
  let color = "var(--color-up)";
  if (value > 4.5) {
    color = "var(--color-down)";
  } else if (value > 2.5) {
    color = "var(--color-warning)";
  }
  return <div className="flex flex-col items-center gap-1 select-none">
...
export const ValueAtRiskWidget = () => {
  ...
  const monteCarloVaR = Math.abs(metrics.maxDrawdown * 0.35);
  return <div className="flex flex-col justify-between h-full gap-2">
